...
class ManagerController extends Controller
{
    public function getProducts()
    {
        ...
        return view('manager.view_all_products')->with($data);
    }
    public function my_shop_orders() {
        $data['include'] = 'manager.sidebar';
        $data['title'] = 'My Shop Orders';
        $shop = ShopWorker::where('user_id', auth()->user()->id)->first();
        $data['orders'] = Order::where('shop_id', $shop->shop_id)->with('user', 'product', 'shop')->orderBy('id', 'desc')->get();
        return view('view_orders')->with($data);
    }
